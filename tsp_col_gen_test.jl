...
# @variable(rmp_τ, λ >= 0)
@variable(rmp_τ, η_ub[2:n]>=0)
@variable(rmp_τ, η_lb[2:n]>=0)
# @variable(rmp_τ, λ[1:7] >= 0)

# Add the objective function
@objective(rmp_τ, Min, sum((α_tilde[i] + ϵ_α)*η_ub[i] - (α_tilde[i] - ϵ_α)*η_lb[i] for i=2:n))
# @objective(rmp_τ, Min, sum(obj_array[i] * λ[i] for i=1:7))

@constraint(rmp_τ, vertex_constraints[i=2:n], η_ub[i] - η_lb[i] == 2)
...
function create_reduced_costs(rmp)
    ...
    if isdefined(:convexity_constraint)
        if (convexity_constraint != nothing)
            α = [getdual(convexity_constraint); α.innerArray]
        else
            α = [0; α.innerArray]
        end
    else
...
function find_one_tree(d_hat, distances; num_trees = 1)
    ...
    if isdefined(:convexity_constraint)
        if (convexity_constraint != nothing)
            temp_dual_array = [getdual(convexity_constraint); new_dual.innerArray]
        else
            temp_dual_array = [0; new_dual.innerArray]
        end
    else
        temp_dual_array = [0; new_dual.innerArray]
    end
    ...
    for i=1:num_trees
        ...
        temp_reduced_cost = temp_cost- sum(temp_num_v[j]*temp_dual_array[j] for j=1:n)
...
function append_new_col!(rmp_τ)
    ...
    for (index, new_col_cost) in enumerate(new_costs)
        if convexity_constraint == nothing
            if !isdefined(:convexity_constraint)
                global convexity_constraint = @constraint(rmp_τ, 0 == 2)
            else
                global convexity_constraint = @constraint(rmp_τ, 0 == 2)
            end
        end
        ...
        @variable(rmp_τ, 0 <= λ_new <= Inf, objective = new_col_cost, inconstraints = constraint_refs, coefficients = num_v_array[index])
...
while gap > ϵ
    ...
    if isdefined(:convexity_constraint)
        if (convexity_constraint != nothing)
            new_dual = [getdual(convexity_constraint); new_dual.innerArray]
        else
            new_dual = [0; new_dual.innerArray]
        end
    else
        new_dual = [0; new_dual.innerArray]
    end
